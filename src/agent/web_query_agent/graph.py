...
from web_query_agent import prompts
from web_query_agent.configuration import Configuration
from web_query_agent.state import InputState, OutputState, State
from web_query_agent.tools import scrape_website, search
from web_query_agent.utils import init_model

class SchemaDesignResult(BaseModel):
    """The result of the schema design agent."""
    extraction_schema: str = Field(description="The Json schema you designed for the web research")

async def schema_design_agent(
    state: State, *, config: Optional[RunnableConfig] = None
) -> SchemaDesignResult:
    """Design a schema for the web research."""
    prompt = """You are designing a schema for a web research agent.

The user is interested in the following question:

{query}

You need to design a schema synthesizing the information from the web search that aligns with the user's question.
You should always include type and description for each field. Do not include examples.
If user specifies any detail they are looking for, you should denote them as required fields.

The schema should be a valid JSON object.
"""
    p = prompt.format(query=state.topic)
    raw_model = init_model(config)
    structured_model = raw_model.with_structured_output(SchemaDesignResult)
    result = await structured_model.ainvoke(p)
    try:
        parsed_result = json.loads(result.extraction_schema)
    except Exception as e:
        raise ValueError(f"Failed to parse the extraction schema: {str(result)}")
    return {
        "extraction_schema": parsed_result
    }
...
workflow.add_node(schema_design_agent)
workflow.add_node(call_agent_model)
workflow.add_node(reflect)
workflow.add_node("tools", ToolNode([search, scrape_website]))
workflow.add_edge("__start__", "schema_design_agent")
workflow.add_edge("schema_design_agent", "call_agent_model")
